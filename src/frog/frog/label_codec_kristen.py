...
class LabelCodec:
    def __init__(self, spelling, mode="legacy", strict=True, use_ambiguous_label=False,
                 use_ambiguous_label_for_key_only=False):
        ...
        self.strict = strict
        self.use_ambiguous_label = use_ambiguous_label
        self.use_ambiguous_label_for_key_only = use_ambiguous_label_for_key_only
        if self.mode == "legacy":
            self.keys = key_temp + [p.lower() for p in key_temp]

            if (self.use_ambiguous_label
                or self.use_ambiguous_label_for_key_only):
                self.keys += ["ambiguous"]

            self.degrees = [alt + deg for alt in DEGREES_ALTERATIONS for deg in DEGREES_DIATONIC]

            if self.use_ambiguous_label:
                self.degrees += ["ambiguous"]
    ...
    def encode_key(self, key):
        if not self.spelling and key != "ambiguous":
    ...
    def encode_degree(self, degree):
        ...
        """
        if degree == "ambiguous":
            tonicisation = self.D2I.get("ambiguous", None)
            degree = self.D2I.get("ambiguous", None)
            return tonicisation, degree
