...
def split_data_into_train_valid_test(datasets, in_folder, out_folder):
    [
...
def _create_tfrecords(in_folder, tfr_base_folder, input_type, output_mode, beat_strength, n_max, compression,
                      chords_folder, data_presplit, use_ambiguous_label, use_ambiguous_label_for_key_only):
    # Preparation
    ...
    lc = LabelCodec(spelling=spelling == "spelling", mode=output_mode, strict=False,
                    use_ambiguous_label=use_ambiguous_label,
                    use_ambiguous_label_for_key_only=use_ambiguous_label_for_key_only)
    datasets = ["train", "valid", "test"]

    if not data_presplit:
        split_data_into_train_valid_test(datasets, in_folder, out_folder)

    files_per_dataset = {}
    for ds in datasets:
        ds_folder = os.path.join(in_folder, ds)
        ds_chords_folder = os.path.join(ds_folder, chords_folder)
        ds_scores_folder = os.path.join(ds_folder, "scores")
        #print("chords folder:", ds_chords_folder)

        if os.path.isdir(ds_chords_folder):
            file_names = [
                ".".join(fn.split(".")[:-1])
                for fn in os.listdir(ds_chords_folder)
            ...
            with tf.io.TFRecordWriter(output_file, options=compression) as writer:
                ...
                for n, fn in enumerate(file_names):
                    if n >= n_max:
                        break
                    sf = os.path.join(ds_scores_folder, fn + ".mxl")
                    cf = os.path.join(ds_chords_folder, fn + ".csv")
...
def main(opts):
    ...
    parser.add_argument("input_types", nargs="+", choices=INPUT_TYPES,
                        help="Input type(s) to use")
    ...
    parser.add_argument("-if", dest="input_folder", default="../data", help="Where to read the input files from")
    parser.add_argument("-of", dest="output_folder", default="../data", help="Where to store the tfrecords")
    parser.add_argument("-no_bs", action="store_true", help="Deactivate the beat strength")
    parser.add_argument("--data_presplit", action="store_true", help="Input data already split into train/validation/test")
    parser.add_argument("-c", default="ZLIB", help="Type of compression to use")
    parser.add_argument("--chords_folder", choices=["chords",
                                                    "basic_key_segment_annotator_chords",
                                                    "mixture_key_segment_annotator_chords",
                                                    "relaxed_key_segment_annotator_chords",
                                                    "strict_key_segment_annotator_chords"
                                                    ],
                        type=str, help="Folder from which to read csv chord files.")
    parser.add_argument("--use_ambiguous_label", action="store_true", help="Use 'ambiguous' label for key, tonicization, and chord degree classes.")
    parser.add_argument("--use_ambiguous_label_for_key_only", action="store_true", help="Use 'ambiguous' label for key class only.")
    args = parser.parse_args(opts)
    for it in args.input_types:
        _create_tfrecords(args.input_folder, args.output_folder, it, args.output_mode, not args.no_bs, args.n, args.c,
                          args.chords_folder, args.data_presplit, args.use_ambiguous_label, args.use_ambiguous_label_for_key_only)
